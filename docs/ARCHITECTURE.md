...
**Логирование.** Каждая ветвь сбоя пишет структурированный лог с контекстом (`notification_id`, `recipient_id`, `channel`, `attempt`, `reason`): `provider rejected the message` (warning) при постоянном отказе, `transient gateway failure, will retry` (warning) перед ретраем, `notification permanently failed` (error) в `failed()` — главная точка для алертов «сообщения не доходят». Канал — `stderr` (логи собирает оркестратор). Логи и тексты исключений — на английском (телеметрия, грепабельность); пользовательские тексты валидации — на русском через `lang/ru/` (см. §10).
...
- `NotificationProviderInterface` — контракт: `send(NotificationMessage): ProviderResponse`; постоянные ошибки — `ProviderRejectedException`, временные — `ProviderTemporarilyUnavailableException` (`app/Exceptions/`)
...
**Локализация.** Пользовательские тексты валидации — на русском через `lang/ru/validation.php`: переведённые правила + секция `attributes` (имена полей) собирают сообщение автоматически, без ручной формулировки на каждое правило; кастомное сообщение про `Idempotency-Key` — через ключ `custom`. `messages()` в FormRequest не используется. `APP_LOCALE=ru` задаётся окружением (docker-compose). Логи и тексты исключений остаются английскими (см. §6).
...
- **Ресурсная изоляция воркеров** (`deploy.resources`): `worker-high` получает резервирование CPU/памяти (гарантированный минимум), `worker-low` ограничен сверху — маркетинговый трафик не может задавить хост и отнять ресурсы у критичного. Дополняет логическую изоляцию очередей: транзакционные сообщения и не ждут в очереди, и не голодают по CPU. Масштабирование под всплеск: `docker compose up -d --scale worker-high=N` (competing consumers — RabbitMQ раздаёт задания между экземплярами без дублей).
...
9. **Автоскейл воркеров не автоматизирован.** Ресурсная и логическая изоляция есть (§11), но при лавине именно критичного трафика `worker-high` масштабируется вручную (`--scale`). В проде — автоскейл по глубине очереди (k8s HPA/KEDA по метрике RabbitMQ).
10. **Фиксированный `APP_KEY` в docker-compose** — см. раздел 11.
